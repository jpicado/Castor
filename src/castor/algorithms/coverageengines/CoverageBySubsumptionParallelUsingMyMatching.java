...
import ida.ilp.logic.subsumption.MyMatching;
...
public class CoverageBySubsumptionParallelUsingMyMatching implements CoverageEngine {
	
	public static double coverageTime = 0;
	public static int coverageCalls = 0;
	public static int repeatedClauses = 0;
	...
	private MyMatching positiveMatching;
	private MyMatching negativeMatching;
	
//	private Set<String> clauses = new HashSet<String>();

	public CoverageBySubsumptionParallelUsingMyMatching(GenericDAO genericDAO, BottomClauseConstructionDAO bottomClauseConstructionDAO, Relation posExamplesRelation, Relation negExamplesRelation, String spName, int iterations, int recall, int maxterms) {
...
//					System.err.println("IDA library failed to parse clause:\n" + groundClause);
		...
		// Initialize matching
		this.positiveMatching = new MyMatching(posExamples);
		this.negativeMatching = new MyMatching(negExamples);
	}
...
//	@Override
	...
	public boolean entails(GenericDAO genericDAO, Schema schema, ClauseInfo clauseInfo, Tuple example, Relation examplesRelation, boolean isPositiveRelation) {
		...
		if (evaluatedExamples[index]) {
			...
			if (subsumptionResult[index] == MyMatching.YES) {
	...
	public List<Tuple> coveredExamplesTuplesFromRelation(GenericDAO genericDAO, Schema schema, ClauseInfo clauseInfo, Relation examplesRelation, boolean isPositiveRelation) {
		...
		for (int i = 0; i < subsumptionResult.length; i++) {
			...
			if (covered[i] || subsumptionResult[i] == MyMatching.YES) {
	...
	private int[] clauseCoverage(MyClause clause, boolean[] undecided, boolean isPositiveRelation) {
		...
		// Evaluate using theta-subsumption
		if (isPositiveRelation) {
			subsumptionResult = positiveMatching.evaluateOnExamplesLocal(convertedClause, undecided);
		} else {
			subsumptionResult = negativeMatching.evaluateOnExamplesLocal(convertedClause, undecided);
		}
		
		CoverageBySubsumptionParallelUsingMyMatching.coverageTime += tw.time();
		CoverageBySubsumptionParallelUsingMyMatching.coverageCalls++;
