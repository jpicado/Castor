...
public class CastorLearnerRandomSampleGeneralization extends CastorLearner {
    ...
    private static Logger logger = Logger.getLogger(CastorLearnerRandomSampleGeneralization.class);

    public CastorLearnerRandomSampleGeneralization(GenericDAO genericDAO, BottomClauseConstructionDAO bottomClauseContructionDAO, BottomClauseGenerator saturator,
...
//				 clauseInfo.setMoreGeneralClause(this.transform(schema, clauseInfo.getClause()));
    ...
    private ClauseInfo learnClause(Schema schema, DataModel dataModel, List<Tuple> remainingPosExamples, List<Tuple> uncoveredPosExamples,
        ...
        //Select pos and neg examples for randomSampling in generalization
        ...
        List<Tuple> sampleNegExamples = selectRandomExamples((int)Math.abs(estimationSample*this.coverageEngine.getAllNegExamples().size()), this.coverageEngine.getAllNegExamples());

        boolean createdNewARMGS = true;
        int iters = 0;
        // Add 1 to scores to count seed example, which is not in remainingPosExamples
        double clauseScore = this.computeRandomSampleScore(schema, samplePosExamples, sampleNegExamples, posExamplesRelation, negExamplesRelation,
                bestARMGs.get(0));
        ...
        if (remainingPosExamples.size() > 0) {
            while (createdNewARMGS) {
                ...
                for (ClauseInfo clauseInfo : bestARMGs) {
                    double score = this.computeRandomSampleScore(schema, samplePosExamples, sampleNegExamples, posExamplesRelation,
                            negExamplesRelation, clauseInfo);
                ...
                clauseScore = this.computeRandomSampleScore(schema, samplePosExamples, sampleNegExamples, posExamplesRelation, negExamplesRelation,
                        bestARMGs.get(0));
        ...
        double bestScore = Double.NEGATIVE_INFINITY;
        for (ClauseInfo clauseInfo : bestARMGs) {
            double score = this.computeRandomSampleScore(schema, samplePosExamples, sampleNegExamples, posExamplesRelation, negExamplesRelation,
                    clauseInfo);
            if (score > bestScore) {
    ...
    /*
    * Select random examples from the given exmaple set
    */
    private List<Tuple> selectRandomExamples(int sampleSize, List<Tuple> exampleSet) {
        List<Tuple> examples = new LinkedList<>();
        Set<Integer> randomPositions = generateListOfRandomNumbers(exampleSet.size(),
                sampleSize);
    ...
    /*
    * Compute score of a clause for randomSample
    */
    private double computeRandomSampleScore(Schema schema, List<Tuple> samplePosExamples, List<Tuple> sampleNegExamples, Relation posExamplesRelation,
                                Relation negExamplesRelation, ClauseInfo clauseInfo) {
        double score;
        if (clauseInfo.getScore() == null) {
            score = evaluator.computeRandomSampleScore(genericDAO, coverageEngine, schema, samplePosExamples, sampleNegExamples,
                    posExamplesRelation, negExamplesRelation, clauseInfo, EvaluationFunctions.FUNCTION.COVERAGE);
        } else {
            score = clauseInfo.getScore();
        }
