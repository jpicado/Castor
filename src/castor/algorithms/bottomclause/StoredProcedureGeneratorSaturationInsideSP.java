...
public class StoredProcedureGeneratorSaturationInsideSP {
	...
	private void generateStoredProcedureVariabilized(String dataset, String spNameTemplate,
		...
		for (Mode mode : modesB) {
			...
			if (!groupedModesStrings.containsKey(mode.getPredicateName())) {
				...
				groupedModesStrings.put(mode.getPredicateName(), groupedModesStrings.get(mode.getPredicateName()) + ";" + mode.toString());
			}
	    ...
	    while (it.hasNext()) {
	    	...
	    	for (Mode mode : pair.getValue()) {
				...
				// Check if already saw another mode with the same predicate name and attribute number
	    	...
	    	// Apply INDs
	    	if (applyInds) {
	    		modesOperationsBuilder.append(followIndChain(schema, modePredicate, new HashSet<String>(), groupedModesStrings, stGroup));
	    	}
	    }
	...
	private String followIndChain(Schema schema, String currentPredicate, Set<String> seenPredicates, Map<String, String> groupedModesStrings, STGroup stGroup) {
	...
	private void generateStoredProcedureGround(String dataset, String spNameTemplate,
			int iterations, Schema schema, Mode modeH, List<Mode> modesB, boolean applyInds) throws Exception {
		...
		for (Mode mode : modesB) {
			...
			if (!groupedModesStrings.containsKey(mode.getPredicateName())) {
				...
				groupedModesStrings.put(mode.getPredicateName(), groupedModesStrings.get(mode.getPredicateName()) + ";" + groundMode);
			}
...
//        String voltDBHome = VOLTDB_HOME;
        if (voltDBHome == null || voltDBHome.isEmpty()) {
        	throw new RuntimeException("VoltDB not found in location given by environment variable " + VOLTDB_HOME_VARIABLE + ".");
        }
        
        // Setup class path and output location
        List<String> optionList = new ArrayList<String>();
        optionList.add("-classpath");
//        optionList.add(System.getProperty("java.class.path") + ":./voltdb.jar" + ":" + voltDBHome + "/lib/*:" + voltDBHome + "/voltdb/*");
        ...
        Iterable<? extends JavaFileObject> compilationUnit
                = fileManager.getJavaFileObjectsFromFiles(Arrays.asList(listOfFiles));
        ...
        if (task.call()) {
			// Create catalog jar
			//createJar(spCompiledLocation); //this is not working, currently calling command jar
        	// Assumes jar is installed in computer. Would be better to create jar programmatically
			try {
