package castor.ddlindextract.main;

import castor.ddlindextract.ddlmodel.TableObject;
import castor.ddlindextract.helper.DDLParser;
...
public class DDLIndMain {
    ...
    public String extractIndFromDDL(String ddl) {
        ...
        String result = FileUtils.writeIndsToJsonFormat(inds, ordinalPos, Constants.FileName.OUTPUT_IND.getValue());
        if (!invalidPks.isEmpty()) {
            logger.info("Invalid primary keys found");
            FileUtils.writeInvalidPksToFile(invalidPks, Constants.FileName.OUTPUT_INVALID_PKS.getValue());
        }
        if (result.equals("Success"))
            return Constants.FileName.OUTPUT_IND.getValue();
        return "Failed";
    }
